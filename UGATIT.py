...
class UGATIT(object) :
    def __init__(self, sess, args):
        ...
        " self.acl_weight = args.acl_weight "
        ...
        print("# sem_weight : ", self.sem_weight)
        print("# pixel_weight : ", self.pixel_weight)
        #print("# acl_weight : ", self.acl_weight)"
        print("# tv_weight : ", self.tv_weight)
    ...
    def generator(self, x_init, reuse=False, scope="generator"):
        ...
        with tf.compat.v1.variable_scope(scope, reuse=reuse) :
            ...
            # Class Activation Map این قسمت باید تغییر کند.
            ...
            attention = heatmap[tf.newaxis, ..., tf.newaxis]
            attention.shape.as_list()
            attention = tf.image.resize(attention, [256,256], method='nearest')
            attention_3D = tf.cast(tf.concat([tf.concat([attention, attention], -1), attention], -1), tf.float32)

            # Gamma, Beta block
            gamma, beta = self.MLP(x, reuse=reuse)

            # Up-Sampling Bottleneck Concatenate
            for i in range(self.n_res):
                #x = adaptive_ins_layer_resblock(x, channel, gamma, beta, smoothing=self.smoothing, scope='adaptive_resblock' + str(i))"
                if i == 1:
                    x = adaptive_ins_layer_skipblock(tf.keras.layers.concatenate([x1,x]), channel, gamma, beta, smoothing=self.smoothing, scope='adaptive_resblock' + str(i))
                elif i == 2:
                    x = adaptive_ins_layer_skipblock(tf.keras.layers.concatenate([x2,x]), channel, gamma, beta, smoothing=self.smoothing, scope='adaptive_resblock' + str(i))
                elif i == 3:
                    x = adaptive_ins_layer_skipblock(tf.keras.layers.concatenate([x3,x]), channel, gamma, beta, smoothing=self.smoothing, scope='adaptive_resblock' + str(i))
                else:
                    x = adaptive_ins_layer_skipblock(tf.keras.layers.concatenate([x4,x]), channel, gamma, beta, smoothing=self.smoothing, scope='adaptive_resblock' + str(i))
            ...
            # AGGAN
            x = tf.add(tf.multiply(attention_3D,x),tf.multiply((1-attention_3D),x))
            #x = (heatmap*x)+((1-heatmap)*x)
            return x, cam_logit, attention_3D, heatmap
    ...
    def discriminator(self, x_init, reuse=False, scope="discriminator"):
        ...
        with tf.compat.v1.variable_scope(scope, reuse=reuse) :
            ...
            D_logit.extend([local_x, global_x])# local_x, global_x میچسبونه آخر آرایه
    ...
    def discriminator_global(self, x_init, reuse=False, scope='discriminator_global'):
        with tf.compat.v1.variable_scope(scope, reuse=reuse):
            ...
            x = lrelu(x, 0.2)
				#تنها تفاوت محلی و جهانی در تعریف حلقه است که در محلی پنجره کوچکتر از کل ولی در جهانی اندازه کل است
    ...
    def build_model(self):
        if self.phase == 'train' :
            ...
            tv_A = tv_loss(attention_a)
            tv_B = tv_loss(attention_b)
            ...
            Generator_A_sem = self.sem_weight * sem_A
            Generator_A_pixel = self.pixel_weight * pixel_A
            #"Generator_A_acl = self.acl_weight * acl_A"
            Generator_A_tv = self.tv_weight * tv_A
            ...
            Generator_B_sem = self.sem_weight * sem_B
            Generator_B_pixel = self.pixel_weight * pixel_B
            #"Generator_B_acl = self.acl_weight * acl_B"
            Generator_B_tv = self.tv_weight * tv_B


            Generator_A_loss = Generator_A_gan + Generator_A_cycle + Generator_A_identity + Generator_A_cam + Generator_A_sem + Generator_A_pixel + Generator_A_tv
            Generator_B_loss = Generator_B_gan + Generator_B_cycle + Generator_B_identity + Generator_B_cam + Generator_B_sem + Generator_B_pixel + Generator_B_tv
            ...
            self.G_A_sem = tf.compat.v1.summary.scalar("G_A_sem", Generator_A_sem)
            self.G_A_pixel = tf.compat.v1.summary.scalar("G_A_pixel", Generator_A_pixel)
            self.G_A_tv = tf.compat.v1.summary.scalar("G_A_tv", Generator_A_tv)
            #self.G_A_acl = tf.summary.scalar("G_A_acl", Generator_A_acl)
            ...
            self.G_B_sem = tf.compat.v1.summary.scalar("G_B_sem", Generator_B_sem)
            self.G_B_pixel = tf.compat.v1.summary.scalar("G_B_pixel", Generator_B_pixel)
            self.G_B_tv = tf.compat.v1.summary.scalar("G_B_tv", Generator_B_tv)
            #self.G_B_acl = tf.summary.scalar("G_B_acl", Generator_B_acl)
            ...
            g_summary_list = [self.G_A_loss, self.G_A_gan, self.G_A_cycle, self.G_A_identity, self.G_A_cam, self.G_A_sem , self.G_A_pixel, self.G_A_tv,
                              self.G_B_loss, self.G_B_gan, self.G_B_cycle, self.G_B_identity, self.G_B_cam, self.G_B_sem , self.G_B_pixel, self.G_B_tv,
                              self.all_G_loss]
    ...
    def model_dir(self):
        ...
        return "{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}{}{}".format(self.model_name, self.dataset_name,
                                                         self.gan_type, n_res, n_dis,
                                                         self.n_critic,
                                                         self.adv_weight, self.cycle_weight, self.identity_weight, self.cam_weight, self.sem_weight, self.pixel_weight, self.tv_weight,
														 sn, smoothing)
	...
	check_folder(self.result_dir + '/fake_B')
        ...
        for sample_file  in test_A_files : # A -> B
            ...
            image_path = os.path.join(self.result_dir + '/fake_A','{0}'.format(os.path.basename(sample_file)))
        ...
        for sample_file  in test_B_files : # B -> A
            ...
            sample_image = np.asarray(load_test_data(sample_file, size=self.img_size))
            image_path = os.path.join(self.result_dir + '/fake_B','{0}'.format(os.path.basename(sample_file)))
