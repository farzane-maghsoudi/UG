...
def fully_connected_with_w(x, use_bias=True, sn=False, reuse=False, scope='linear'):
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        w = tf.compat.v1.get_variable("kernel", [channels, 1], tf.float32,
        ...
        if use_bias:
            weights = tf.gather(tf.transpose(a=tf.nn.bias_add(w, bias)), 0)
        else:
            weights = tf.gather(tf.transpose(a=w), 0)
...
def adaptive_ins_layer_resblock(x_init, channels, gamma, beta, use_bias=True, smoothing=True,
...
def adaptive_ins_layer_skipblock(x_init, channels, gamma, beta, use_bias=True, smoothing=True,
                                 scope='adaptive_resblock'):
    with tf.compat.v1.variable_scope(scope):
        with tf.compat.v1.variable_scope('res1'):
            x = conv(x_init, channels, kernel=3, stride=1, pad=1, pad_type='reflect', use_bias=use_bias)
...
def global_max_pooling(x):
    gmp = tf.reduce_max(input_tensor=x, axis=[1, 2])
    return gmp
...
def adaptive_instance_layer_norm(x, gamma, beta, smoothing=True, scope='instance_layer_norm'):
    with tf.compat.v1.variable_scope(scope):
        ...
        ins_mean, ins_sigma = tf.nn.moments(x=x, axes=[1, 2], keepdims=True)
        x_ins = (x - ins_mean) / (tf.sqrt(ins_sigma + eps))

        ln_mean, ln_sigma = tf.nn.moments(x=x, axes=[1, 2, 3], keepdims=True)
        x_ln = (x - ln_mean) / (tf.sqrt(ln_sigma + eps))

        rho = tf.compat.v1.get_variable("rho", [ch], initializer=tf.compat.v1.constant_initializer(1.0),
...
def layer_instance_norm(x, scope='layer_instance_norm'):
    with tf.compat.v1.variable_scope(scope):
        ...
        rho = tf.compat.v1.get_variable("rho", [ch], initializer=tf.compat.v1.constant_initializer(0.0),
                                        constraint=lambda x: tf.clip_by_value(x, clip_value_min=0.0,
                                                                              clip_value_max=1.0))

        gamma = tf.compat.v1.get_variable("gamma", [ch], initializer=tf.compat.v1.constant_initializer(1.0))
        beta = tf.compat.v1.get_variable("beta", [ch], initializer=tf.compat.v1.constant_initializer(0.0))
...
def spectral_norm(w, iteration=1):
    ...
    u = tf.compat.v1.get_variable("u", [1, w_shape[-1]], initializer=tf.compat.v1.random_normal_initializer(),
                                  trainable=False)
...
def tv_loss(x):
    ...
    w_tv = tf.reduce_sum(input_tensor=tf.pow((x[:, :, 1:, :] - x[:, :, :w_x - 1, :]), 2))
    tv_w = 10
    loss = tv_w * 2 * ((h_tv / count_h) / (w_tv / count_w)) / batch_size
    return loss


def cam_loss(source, non_source):
    identity_loss = tf.reduce_mean(
        input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(source), logits=source))
    non_identity_loss = tf.reduce_mean(
        input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(non_source), logits=non_source))
...
def discriminator_loss(loss_func, real, fake):
    ...
    for i in range(2):
        if loss_func.__contains__('wgan'):
            real_loss = -tf.reduce_mean(input_tensor=real[i])
            fake_loss = tf.reduce_mean(input_tensor=fake[i])
        ...
        if loss_func == 'gan' or loss_func == 'dragan':
            real_loss = tf.reduce_mean(
                input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(real[i]), logits=real[i]))
            fake_loss = tf.reduce_mean(
                input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(fake[i]), logits=fake[i]))

        if loss_func == 'hinge':
            real_loss = tf.reduce_mean(input_tensor=relu(1.0 - real[i]))
            fake_loss = tf.reduce_mean(input_tensor=relu(1.0 + fake[i]))
...
def generator_loss(loss_func, fake):
    ...
    for i in range(2):
        ...
        if loss_func == 'gan' or loss_func == 'dragan':
            fake_loss = tf.reduce_mean(
                input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(fake[i]), logits=fake[i]))
